...
function DefaultBanner({options, imageURL, name, colors, authorFName, authorLName, authorImage, authorProfileDesc, authorURL}) {
	...
	return (
                ...
                <div>
                    ...
                    ? {} 
                    : {background:getColor2(colors,1)}} 
                    className={checkOptionSetting(170, 1, options) ? styles.fullImageBanner : styles.mainbannertop}>
                    
                        <div className={`${styles.imagebanner} ${checkOptionSetting(170, 1, options) ? styles.frontshortBanner : styles.fronttallBanner}`}>

                            {(checkOptionSetting(170, 1, options) && imageURL !== '') ?
                            <img src={(imageURL !== '') ? imageURL  :topbannerimage } alt={name} />
                            :
