...
import { checkOptionLanding, getColor } from '../../Context/functions';
import styles from './evalinator.module.css';

function EvalRangeSlider({handleNext,centerAlign,row,currentQuestion,questionOptions,setQuestionOptions}) {
    let list = []
    ...
    if(row.glistQuestions[currentQuestion] && Object.keys(row.glistQuestions[currentQuestion].m_dictCustomizations).length > 0){
    for (var i=1; i <= (row.glistQuestions[currentQuestion].m_dictCustomizations && parseInt(row.glistQuestions[currentQuestion].m_dictCustomizations[3])); i++) {
        list.push(i)        
    }
    }else{
    if(list.length === 0){
   ...
   // console.log("questionOptions",questionOptions);
    ...
    return (
        ...
        <div className={`${styles.rangeslidercustom} ${centerAlign !== true ? styles.contentCenter:''}`}>
            ...
            <div className={styles.range_slide_options}>
                ...
                { list.length > 0 && list.map((obj,index) => {
                    return <li  key={index}>
                        <input type={"radio"} name={`radioOption-${row && row.glistQuestions[currentQuestion].m_lQuestionId}-2`}  checked={(obj+"" === questionOptions[row.glistQuestions[currentQuestion].m_lQuestionId]) ? true : false} value={obj}  onChange={(e)=>{
        ...
        <>
        <RangeSlider 
        value={((questionOptions && questionOptions[row.glistQuestions[currentQuestion].m_lQuestionId] !== undefined) ? questionOptions[row.glistQuestions[currentQuestion].m_lQuestionId] : 1)}       
        ...
        onChange={(e)=>{setQuestionOptions({...questionOptions,[row.glistQuestions[currentQuestion].m_lQuestionId]:e.target.value });
        if(checkOptionLanding(150,1,row)){
            setTimeout(function(){handleNext();},500)
        }
        ...
        <div className={styles.range_slide_option}>
            { list.length > 0 && list.map((obj,index) => {
                return <span key={index} style={{backgroundColor: (questionOptions[row.glistQuestions[currentQuestion].m_lQuestionId] && questionOptions[row.glistQuestions[currentQuestion].m_lQuestionId].toString() === obj.toString()) ? getColor(row,2) : ""}}
...
export default EvalRangeSlider;